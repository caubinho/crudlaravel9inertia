...
const form = useForm({
    ...
    stripe_id: props.datos.stripe_id,
    description: props.datos.description,
    price: props.datos.price,
    recomended: props.datos.recomended,
});

const submit = () => {
    console.log(form);
    form.put(route('plans.update', props.datos.id));
};
</script>
            ...
            <div class="grid grid-cols-10  gap-4">
                <div class="col-span-6">
                    <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                        Editar Plan - <strong>{{ this.props.datos.name }}</strong>
                    </h2>
                </div>
                <div class="col-end-12 sm:col-end-10">
                    <Link :href="route('plans.index')" class="bg-blue-600  hover:bg-blue-500 text-white  font-semibold py-2 px-4 border border-gray-400 rounded shadow">
                        Volver
                    </Link>
                </div>
            </div>
                                    ...
                                    <form @submit.prevent="submit">
                                        ...
                                        <div class="mb-6">
                                            <div class="grid md:grid-cols-2 text-sm">
                                                <div class="grid grid-cols-4">
                                                    ...
                                                    />
                                                    <div v-if="form.errors.stripe_id" class="col-span-2 text-red-600 text-sm">{{ form.errors.stripe_id }}</div>
                                                </div>
                                                <div class="grid grid-cols-4">
                                                    <label
                                                        for="slug"
                                                    ...
                                                    />
                                                    <div v-if="form.errors.price" class="col-span-2 text-red-600 text-sm">{{ form.errors.price }}</div>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div class="mb-6">
                                            <div class="form-check">
                                                <input v-model="form.recomended" v-if="1" checked class="form-check-input appearance-none h-4 w-4 border border-gray-300 rounded-sm bg-white checked:bg-blue-600 checked:border-blue-600 focus:outline-none transition duration-200 mt-1 align-top bg-no-repeat bg-center bg-contain float-left mr-2 cursor-pointer" type="checkbox" value="1" id="flexCheckDefault">
                                                <input v-model="form.recomended" v-else class="form-check-input appearance-none h-4 w-4 border border-gray-300 rounded-sm bg-white checked:bg-blue-600 checked:border-blue-600 focus:outline-none transition duration-200 mt-1 align-top bg-no-repeat bg-center bg-contain float-left mr-2 cursor-pointer" type="checkbox" value="1" id="flexCheckDefault">
